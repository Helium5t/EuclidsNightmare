...
public class FMODStudioFirstPersonFootsteps : MonoBehaviour
{
    ...
    [SerializeField] [EventRef] private string FootstepsEventPath; 
    ...
    private float StepRandom; // This will be set as random number, which will later be added to the StepDistance to add a little variation to the length in steps.
    private Vector3 PrevPos; // This will hold the co-ordinates of the previous position the player was in during the last frame.
    ...
    private bool isPlayerTouchingGround; // This will hold a value to represent whether or not the player is touching the ground.
    ...
    private void Start() 
    {
        ...
        breathingEventInstance = FMODUnity.RuntimeManager.CreateInstance(GameSoundPaths.BreathingEventPath);
        RuntimeManager.AttachInstanceToGameObject(breathingEventInstance,transform,GetComponent<Rigidbody>());
